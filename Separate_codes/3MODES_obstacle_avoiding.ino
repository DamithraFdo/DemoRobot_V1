...
void analyzeFreePath() {
 ...
 void ModeC(){
  ...
  if(val0 && !val1 && val2) //forward (1&0&1)
  {
    bool x=true;
    InALine(x); 
  }
  else if(!val0 && !val1 && val2)  //turn right (1&0&0)
  {
    bool y=true;
    Turn(y);
  }
  else if(val0 && !val1 && !val2)//turn left (0&1&1)
  {
    bool y=false;
    Turn(y);
  }
  else if(val0 && val1 && val2) //stop (1&1&1)
  {
    stop();
  }
  else if(!val0 && !val1 && !val2) //forward (0&0&0)
  {
    bool x=true;
    InALine(x);
  }
